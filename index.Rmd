...
## Snakemake workflow for metadata acquisition 

```{bash ruledag, include=FALSE}
bash workflow/scripts/rules_dag.sh
```

![A tentative visualization of the snakemake workflow that defines **read quality control rule** in a DAG (directed acyclic graph) format.](dags/rulegraph.svg). 
A detailed interactive snakemake report is available [here](file:///Users/tmbuza/Dropbox/MICROBIOME/imap-read-quality-control/report.html).

<br>

<br><hr width=100%><br>

```{r child='workflow/scripts/01_read_statistics.Rmd'}
```

<br>

```{r child='workflow/scripts/02_quality_inspection.Rmd'}
```
...
> We can view the interactive snakemake HTML report by opening the `report.html` using any compatible browser. You will be able to explore the workflow and the associated statistics. You can close the left bar to get a more expansive display view.
